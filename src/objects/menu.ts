...
import { WidgetCollector } from "./widget/widget-collector";
import { SignalWidget } from "./widget/signal-widget";

export class Menu {
    p: AnimationEditor;
    private _enabled: boolean = true;
    lastSelectedFile: string = "";
    newTileHovered: boolean = false;
    ...
    constructor() {
        ...
        this.computeSize();
        const sound = this.p.files.sound;
    ...
    text(imageName: string, x: number, y: number, editingText: string = "") {
        ...
        this.p.text(
            lookupFiletype(
                Number(imageName.substring(0, imageName.indexOf(".")))
            ) + editingText,
            0,
    ...
    image(
        ...
        j: number,
        isLastTile: boolean
    ): [soundName: string, associatedImageName: string, x: number, y: number] {
        ...
        const sound = this.p.files.sound;
        const newSoundIndex = this.lastSoundIndex;
        const soundName = sound[index] ?? (newSoundIndex + ".wav");
    ...
    drawItems() {
        ...
        for (let i = 0; i <= this.rows; i++) {
            for (let j = 0; j <= this.cols; j++) {
                ...
                if (j === this.cols && index !== this.p.maxSounds) continue;
                // console.log(i, j, this.cols, index, this.p.maxSounds);
                if (index > this.p.maxSounds) return;

                // is the user hovering the last tile?
                const isLastTile = index === this.p.maxSounds;
                const [soundName, associatedImageName, x, y] = this.image(
                    index,
                    i,
                    j,
                    isLastTile
                ...
                } else this.newTileHovered = false;
